...
Alongside hall skills, `ivar` ships built-in **shipped skills** (such as `ivar-execute`, `ivar-plan`, and `ivar-deliver`) and **workflow commands** (such as `/ivar-sync` and other official workflows).
