...
impl ScopeName {
    ...
    pub fn new(raw: impl Into<String>) -> Result<Self, Failure> {
        let s = raw.into();
        if s.is_empty() {
            return Err(Failure::blocked("scope_name.empty", "validating scope name")
                .expected("a non-empty scope identifier")
                .actual("an empty string")
                .fix(FixAction::safe("specify_scope_name", "Provide a non-empty kebab-case scope identifier.")));
        }

        if s == "." || s == ".." || s.contains('/') || s.contains('\\') {
            return Err(Failure::blocked("scope_name.invalid_segment", "validating scope name")
                .expected("a single path segment without traversal")
                .actual(format!("'{s}' contains traversal or path separators"))
                .fix(FixAction::safe("use_single_segment", "Use a single kebab-case segment like 'product' or 'team-conventions'.")));
        }
        ...
        if !is_kebab {
            return Err(Failure::blocked("scope_name.invalid_kebab", "validating scope name")
                .expected("a kebab-case identifier (e.g. 'product', 'team-conventions')")
                .actual(format!("'{s}' is not valid kebab-case"))
                .fix(FixAction::safe("use_kebab_case", "Ensure the name uses only lowercase letters, numbers, and single hyphens.")));
        }
    ...
    pub fn validate(&self) -> Result<(), Failure> {
        ...
        for scope in &self.scopes {
            if !seen.insert(&scope.id) {
                return Err(Failure::blocked("memory_config.duplicate_scope", "validating memory configuration")
                    .expected("unique scope identifiers")
                    .actual(format!("duplicate scope '{}'", scope.id))
                    .fix(FixAction::safe("deduplicate_scopes", "Remove or rename the duplicate scope.")));
            }

            if scope.budget == 0 {
                return Err(Failure::blocked("memory_config.zero_budget", "validating memory configuration")
                    .expected("a positive character budget > 0")
                    .actual(format!("scope '{}' has budget 0", scope.id))
                    .fix(FixAction::safe("set_positive_budget", "Set a positive character budget for the scope.")));
            }
        }
        Ok(())
    }
