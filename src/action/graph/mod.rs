...
fn open_graph_db(ctx: &Ctx) -> Result<GraphDb, Failure> {
    ...
    GraphDb::open(db_path.as_std_path()).map_err(|err| {
        Failure::failed("graph.db_open_failed", format!("Failed to open graph database at {db_path}: {err}"))
    })
}
...
pub fn explore_cmd(ctx: &Ctx, args: ExploreInput) -> Outcome<ExploreOutcome> {
    ...
    let layout = discover_hall(ctx)?;
    let result = explore::explore(&db, layout.root().as_std_path(), &args.query, args.repo.as_deref())
    ...
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        ...
        if res.primary_symbols.is_empty() {
            ...
            for sym in &res.primary_symbols {
                writeln!(w, "    - {} ({:?}) in {}:{}-{}", sym.symbol.name, sym.symbol.kind, sym.file_path, sym.start_line, sym.end_line)?;
        ...
        if !res.call_flows.is_empty() {
            ...
            for flow in &res.call_flows {
                writeln!(w, "    - {} -> {} ({:?}, line {})", flow.caller, flow.callee, flow.edge_kind, flow.line)?;
            }
        }
...
impl WriteHuman for AffectedOutcome {
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        ...
        writeln!(w, "Affected test files ({}):", res.affected_test_files.len())?;
        ...
        if let Some(res) = &self.0 {
            ...
            if res.steps.is_empty() {
                ...
                for (idx, step) in res.steps.iter().enumerate() {
                    writeln!(w, "  {}. {} -> {} ({:?})", idx + 1, step.source, step.target, step.edge_kind)?;
                }
            }
        } else {
            writeln!(w, "No path found.")?;
...
impl WriteHuman for FindOutcome {
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        writeln!(w, "Symbols matching `{}` ({} found):", self.query, self.symbols.len())?;
        for sym in &self.symbols {
            let id_str = sym.symbol.id.map(|id| format!("[#{id}] ")).unwrap_or_default();
            writeln!(
                w,
                "  - {}{}:{:?} in {}/{} ({}:{})",
                id_str,
...
impl WriteHuman for CallersOutcome {
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        writeln!(w, "Callers of `{}` ({} found):", self.symbol, self.callers.len())?;
...
impl WriteHuman for CalleesOutcome {
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        writeln!(w, "Callees of symbol ID {} ({} found):", self.symbol_id, self.callees.len())?;
        for callee in &self.callees {
            let target_name = callee.callee_symbol.as_ref().map(|c| c.name.as_str()).unwrap_or(&callee.callee_name);
            let target_kind = callee.callee_symbol.as_ref().map(|c| format!(" ({:?})", c.kind)).unwrap_or_default();
            writeln!(
                w,
                "  - {}{} (line {}, kind: {:?}, prov: {:?})",
                target_name,
...
impl WriteHuman for FileOutcome {
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        let outline = &self.0;
        writeln!(w, "File outline for {}/{} ({} symbols):", outline.repo, outline.file_path, outline.symbols.len())?;
...
pub fn index_cmd(ctx: &Ctx, args: IndexInput) -> Outcome<IndexBatchOutcome> {
    ...
    if let Some(target_repo) = args.repo {
        let repo_decl = manifest.repos().iter().find(|r| r.name().as_str() == target_repo).ok_or_else(|| {
            Failure::blocked("repo.not_found", format!("Repository `{target_repo}` not found in manifest"))
        })?;
        ...
        if !Path::new(repo_path.as_std_path()).exists() {
            return Err(Failure::blocked(
                ...
                format!("Repository worktree does not exist at `{repo_path}`. Run `ivar sync` first."),
            ));
        }
        let outcome = index::index_repo(&db, repo_decl.name().as_str(), repo_path.as_std_path())
    ...
    // After indexing repos, run cross-repo edge resolution
    ...
    Ok(Report::new(IndexBatchOutcome {
        repos: outcomes,
        cross_edges_linked: cross_edges.total_linked,
    }))
}
    ...
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        ...
        for outcome in &self.repos {
            if outcome.skipped_up_to_date {
                writeln!(w, "  - {}: up to date (took {}ms)", outcome.repo, outcome.duration_ms)?;
            } else {
                writeln!(
                    w,
...
impl WriteHuman for ImpactOutcome {
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        let res = &self.0;
        writeln!(w, "Blast Radius Impact Analysis for symbol `{}` (ID: {}):", res.root_symbol.name, res.root_symbol.id.unwrap_or(0))?;
...
// 11. mcp
pub fn mcp_cmd(_ctx: &Ctx) -> Outcome<McpOutcome> {
...
impl WriteHuman for McpOutcome {
    fn write_human(&self, w: &mut impl io::Write) -> io::Result<()> {
        writeln!(w, "MCP server placeholder.")
    }
}
