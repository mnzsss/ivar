...
pub fn auto_commit_memory(
    ...
    let temp_dir_path = root.as_std_path().join(format!(".ivar-git-index-{}", uuid::Uuid::new_v4()));
    ...
    let head_output = proc::capture(&git_cmd(&["rev-parse", "HEAD"])).map_err(|err| {
        Failure::failed("git.memory_commit.exec_failed", format!("git rev-parse HEAD failed: {err}"))
    })?;
    let head_sha = if head_output.success() {
        let sha = head_output.stdout.trim().to_string();
    ...
    if let Some(head) = &head_sha {
        let read_tree = proc::capture(&git_cmd(&["read-tree", head])).map_err(|err| {
            Failure::failed("git.memory_commit.exec_failed", format!("git read-tree failed: {err}"))
        })?;
    ...
    let add_out = proc::capture(&git_cmd(&add_args)).map_err(|err| {
        Failure::failed("git.memory_commit.exec_failed", format!("git add failed: {err}"))
    })?;
    if !add_out.success() {
        return Err(Failure::failed(
    ...
    let write_tree_out = proc::capture(&git_cmd(&["write-tree"])).map_err(|err| {
        Failure::failed("git.memory_commit.exec_failed", format!("git write-tree failed: {err}"))
    })?;
    ...
    let tree_sha = write_tree_out.stdout.trim().to_string();
    ...
    if let Some(head) = &head_sha {
        let head_tree_out = proc::capture(&git_cmd(&["rev-parse", &format!("{head}^{{tree}}")]))
            .map_err(|err| {
                Failure::failed("git.memory_commit.exec_failed", format!("git rev-parse tree failed: {err}"))
            })?;
    ...
    let commit_out = proc::capture(&git_cmd(&commit_args)).map_err(|err| {
        Failure::failed("git.memory_commit.exec_failed", format!("git commit-tree failed: {err}"))
    })?;
    ...
    let new_commit_sha = commit_out.stdout.trim().to_string();
    ...
    let update_out = proc::capture(&git_cmd(&update_args)).map_err(|err| {
        Failure::failed("git.memory_commit.exec_failed", format!("git update-ref failed: {err}"))
    })?;
