...
#[test]
fn accepted_revision_pins_normalized_plan_fingerprint() {
    ...
    start_run(&ctx, &feature, &plan, false);

    fs::write_text(&plan, "# Plan\n\n- [ ] Revised task\n").unwrap();
    let first_report = write_report(&layout, &feature);
    let divergence = finish(
        &ctx,
        FinishInput {
            feature: feature.to_string(),
            plan: plan.to_string(),
    ...
    .expect_err("semantic revision must diverge before acceptance");
    assert_eq!(divergence.code, "execute.plan_diverged");
    ...
    .unwrap();
    start_run(&ctx, &feature, &plan, true);
    fs::write_text(&plan, "# Plan\n\n- [x] Revised task\n").unwrap();

    let final_report = write_report(&layout, &feature);
    let outcome = finish(
        &ctx,
        FinishInput {
            feature: feature.to_string(),
    ...
    .expect("checkbox progress must not diverge an accepted revision");
