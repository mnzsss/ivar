...
fn test_clean_feature_and_stats_reporting() {
    ...
    db.insert_repo("core", "/base/core", "main", Some("c1"))
        .unwrap();
    let l1 = db
        .ensure_layer_record("feat-1", "core", "/feat1/core", "c1")
        .unwrap();
    let r1 = format!("core/{l1}");
    db.insert_repo(&r1, "/feat1/core", "feat-1", Some("c1"))
        .unwrap();
    db.upsert_file(&r1, "src/a.rs", "h1", 10, 10).unwrap();

    let l2 = db
        .ensure_layer_record("feat-2", "core", "/feat2/core", "c1")
        .unwrap();
    let r2 = format!("core/{l2}");
    db.insert_repo(&r2, "/feat2/core", "feat-2", Some("c1"))
        .unwrap();
