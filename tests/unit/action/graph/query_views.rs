...
fn test_query_reads_reflect_session_layer_and_hide_shadowed_symbols() {
    ...
    db.insert_repo("backend", "/base/backend", "main", Some("c1"))
        .unwrap();
    let f1 = db
        .upsert_file("backend", "src/auth.rs", "h1", 100, 100)
        .unwrap();
    db.insert_symbols(&[Symbol {
        id: None,
        file_id: Some(f1),
        repo: "backend".to_owned(),
        name: "old_login".to_owned(),
        kind: SymbolKind::Fn,
        scope: None,
        signature: Some("pub fn old_login()".to_owned()),
        docstring: None,
    ...
    db.insert_repo("backend/1", "/feat/backend", "feat", Some("c2"))
        .unwrap();
    let f2 = db
        .upsert_file("backend/1", "src/auth.rs", "h2", 200, 200)
        .unwrap();
    db.insert_symbols(&[Symbol {
        id: None,
        file_id: Some(f2),
        repo: "backend/1".to_owned(),
        name: "login_v2".to_owned(),
        kind: SymbolKind::Fn,
        scope: None,
        signature: Some("pub fn login_v2()".to_owned()),
        docstring: None,
        span: Span::new(1, 1, 5, 1),
        is_exported: true,
        complexity: Some(1),
    ...
    db.configure_session_mode(&[("backend", "backend/1")])
        .unwrap();
    ...
    assert_eq!(
        base_syms.len(),
        0,
        "Base symbol 'old_login' must be shadowed"
    );
    ...
    assert_eq!(
        feat_syms[0].symbol.repo, "backend",
        "Symbol repo must be projected as canonical name"
    );
