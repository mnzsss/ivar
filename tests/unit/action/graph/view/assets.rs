...
fn assets_are_embedded_and_non_empty() {
    assert!(
        !INDEX_HTML.trim().is_empty(),
        "INDEX_HTML must not be empty"
    );
    assert!(
        !VIEWER_CSS.trim().is_empty(),
...
fn html_contains_required_dom_structure_and_accessibility_attributes() {
    ...
    assert!(
        INDEX_HTML.contains(r#"<meta charset="utf-8">"#)
            || INDEX_HTML.contains(r#"<meta charset="UTF-8">"#)
    );
...
fn css_declares_fira_code_and_heimdall_palette_tokens() {
    ...
    assert!(
        VIEWER_CSS.contains("font-family: 'Fira Code'")
            || VIEWER_CSS.contains("font-family: \"Fira Code\"")
    );
    assert!(
        VIEWER_CSS.contains(r#"url('/fonts/fira-code-400.woff2')"#)
            || VIEWER_CSS.contains(r#"url("/fonts/fira-code-400.woff2")"#)
    );
    ...
    assert!(
        VIEWER_CSS.contains("@media (prefers-reduced-motion: reduce)")
            || VIEWER_CSS.contains("prefers-reduced-motion")
    );
}
...
fn js_uses_fixed_api_routes_and_no_continuous_animation() {
    ...
    assert!(
        VIEWER_JS.contains("setLineDash")
            || VIEWER_JS.contains("[4, 4]")
            || VIEWER_JS.contains("[6, 4]")
    );
