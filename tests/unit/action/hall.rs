...
fn migrate_on_a_current_hall_reports_nothing_to_do_and_writes_nothing() {
    ...
    assert_eq!(report.value.plan, MigrationPlan::Current { version: 4 });
...
fn migrate_refuses_a_file_newer_than_this_build_without_touching_it() {
    ...
    assert!(human(&report.value).contains("understands up to 4"));
...
fn migrate_reports_an_unversioned_file_as_unreachable_rather_than_adopting_it() {
    ...
    assert_eq!(
        report.value.plan,
        MigrationPlan::Unreachable { from: 0, to: 4 }
    );
    assert!(!report.value.migrated);
    assert!(human(&report.value).contains("no migration to reach version 4"));
    assert!(
        !report.is_clean(),
