...
fn materialise_view_dir_symlinks_hall_commands_into_the_config_dir() {
    ...
    assert!(
        fs::is_file(&commands_link.join("ivar-deliver.md")).unwrap(),
...
fn omp_session_view_dir_materialises_commands_and_hooks() {
    ...
    assert!(
        crate::infra::fs::is_file(&commands_link.join("ivar-deliver.md")).unwrap(),
...
fn omp_sync_removes_stale_managed_commands_and_preserves_user_files() {
    ...
    assert!(crate::infra::fs::is_file(&omp_commands.join("ivar-deliver.md")).unwrap());
