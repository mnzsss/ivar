...
fn test_rename_and_drop_feature_layers_in_graph_db() {
    ...
    db.insert_repo("core", "/base/core", "main", Some("c1"))
        .unwrap();
    let l1 = db
        .ensure_layer_record("feat-alpha", "core", "/feat-a/core", "c1")
        .unwrap();
    let layer_repo = format!("core/{l1}");
    db.insert_repo(&layer_repo, "/feat-a/core", "feat-alpha", Some("c1"))
        .unwrap();
    ...
    let rec = db
        .get_layer_record("feat-beta", "core")
        .unwrap()
        .expect("renamed record exists");
    ...
    let l2 = db
        .ensure_layer_record("feat-stale", "core", "/stale/core", "c1")
        .unwrap();
    db.insert_repo(
        &format!("core/{l2}"),
        "/stale/core",
    ...
    .unwrap();
    let l3 = db
        .ensure_layer_record("feat-active", "core", "/active/core", "c1")
        .unwrap();
    db.insert_repo(
        &format!("core/{l3}"),
        "/active/core",
    ...
    assert!(
        db.get_layer_record("feat-active", "core")
            .unwrap()
            .is_some()
    );
}
