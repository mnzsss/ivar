...
fn test_visible_views_base_and_session_shadowing() {
    ...
    let f_base1 = db.upsert_file("core", "src/lib.rs", "hash_base", 100, 100).unwrap();
    let f_base2 = db.upsert_file("core", "src/deleted.rs", "hash_del", 100, 100).unwrap();
    db.insert_symbols(&[
        Symbol {
            id: None,
    ...
    let base_files: Vec<String> = db.conn().prepare("SELECT path FROM visible_files WHERE repo = 'core' ORDER BY path")
        .unwrap()
    ...
    let layer_id = db.ensure_layer_record("feature-a", "core", "/feat/core", "c1").unwrap();
    assert_eq!(layer_id, 1);
    let layer_repo = format!("core/{layer_id}");
    db.insert_repo(&layer_repo, "/feat/core", "feature-a", Some("c1_feat")).unwrap();
    let f_layer1 = db.upsert_file(&layer_repo, "src/lib.rs", "hash_feat", 200, 200).unwrap();
    let f_layer3 = db.upsert_file(&layer_repo, "src/new.rs", "hash_new", 200, 200).unwrap();
    db.insert_symbols(&[
        Symbol {
            id: None,
    ...
    db.set_layer_tombstones(layer_id, &["src/deleted.rs"]).unwrap();
    ...
    let visible_paths: Vec<String> = db.conn().prepare("SELECT path FROM visible_files WHERE repo = 'core' ORDER BY path")
        .unwrap()
        .query_map([], |row| row.get(0)).unwrap()
        .map(|r| r.unwrap()).collect();
    ...
    let visible_syms: Vec<(String, String)> = db.conn().prepare("SELECT repo, name FROM visible_symbols WHERE repo = 'core' ORDER BY name")
        .unwrap()
        .query_map([], |row| Ok((row.get(0)?, row.get(1)?))).unwrap()
        .map(|r| r.unwrap()).collect();
    assert_eq!(visible_syms, vec![("core".into(), "feat_fn".into()), ("core".into(), "new_fn".into())]);
}
