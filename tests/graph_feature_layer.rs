...
fn e2e_feature_session_reads_its_checkout_and_refreshes_dirty_edits() {
    ...
    assert!(hall.graph_command(&view, &["find", "alpha_fn", "--json"])["matches"].as_array().unwrap().is_empty());
    assert!(hall.graph_command(&view, &["find", "old_fn", "--json"])["matches"].as_array().unwrap().is_empty());
    assert_eq!(hall.graph_command(&view, &["find", "new_fn", "--json"])["matches"][0]["name"], "new_fn");

    let explored = hall.graph_command(&view, &["explore", "beta_fn", "--json"]);
    assert!(explored["sources"][0]["content"].as_str().unwrap().contains("pub fn beta_fn()"));

    let base_find = hall.graph_command(hall.root(), &["find", "alpha_fn", "--json"]);
    assert_eq!(base_find["matches"][0]["name"], "alpha_fn");

    hall.write(&worktree, "src/lib.rs", "pub fn zeta_fn() -> i32 { 20 }\npub fn keep_fn() {}\n");
    let dirty_find = hall.graph_command(&view, &["find", "zeta_fn", "--json"]);
    assert_eq!(dirty_find["matches"][0]["name"], "zeta_fn");
    assert!(hall.graph_command(&view, &["find", "beta_fn", "--json"])["matches"].as_array().unwrap().is_empty());

    hall.graph_command(hall.root(), &["clean", "--feature", "rename-feat", "--json"]);
    let stats = hall.graph_command(hall.root(), &["stats", "--json"]);
    assert!(stats["layers"].as_array().unwrap().iter().all(|layer| layer["feature"] != "rename-feat"));
}
